...
var testSong: [50]u16 = undefined;
var testSongInitialized = false;

fn getNote(pos: usize, song: [*]u16, songLen: usize) Note {
...
export fn sfxBuffer(u8Array: [*]u8, u8ArrayLength: usize, sampleRate: u32, songNotes: [*]u16, songNotesLength: usize, bugLength: u32) void {
    var chosenSong = songNotes;
    var chosenSongLength = songNotesLength;
    if (songNotesLength == 0) {
        chosenSong = &testSong;
        chosenSongLength = testSong.len;
    }
    if (!testSongInitialized) {
        ...
        for (testSong[0..testSong.len]) |*val, pos| {
            val.* = NoteHz[@intCast(u8, pos % (max_note - min_note)) + min_note];
        }
    }
    ...
    var note = getNote(song_index, chosenSong, chosenSongLength);
