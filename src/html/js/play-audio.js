...
function __parseConstants(callback){
    ...
    const bugLengthEl = document.getElementById("bug-length");
    ...
    const bugLength = bugLengthEl.value - 0;
    const secondsLength = secondsLengthEl.value - 0;
    if (sampleRate == 0) {
	sampleRate = 44100;
    }
    callback(sampleRate, songNotes, secondsLength, bugLength);
...
async function main(){
    ...
    */
    function playSoundEffect(sampleRate, songNotes, secondsLength, bugLength) {
	...
	const inputSong = new Uint16Array(memory.buffer, allocatorIndex, songNotes.length);
	for (var i = 0; i <  songNotes.length; ++i) {
	    inputSong[i] = songNotes[i];
	}
	allocatorIndex += songNotes.length * 2;
...
async function downloadWav(sampleRate, songNotes, secondsLength, bugLength){
    let allocatorIndex=0;
    const synthWASMModule = await synthWASMModulePromise;
    const {memory, sfxBuffer, u8ArrayToF32Array} = synthWASMModule;
